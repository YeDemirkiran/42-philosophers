...
void	philo_clear_and_exit(t_philosopher *philo, int status)
{
	...
	sem_close(philo->death_semaphore);
	if (philo->meal_semaphore != NULL)
		sem_close(philo->meal_semaphore);
	exit(status);
}
...
t_byte	should_philo_continue(t_philosopher *philo)
{
	...
	if (time_2 - time > 5)
		return (0);
	else if (time - philo->last_meal_time >= (long)philo->config->starve_time)
		return (0);
	return (1);
}
...
t_byte	take_forks(t_philosopher *philo)
{
	...
	{
		if (interval_sleep(philo->config->starve_time, philo) != 1)
			return (0);
		sem_post(philo->forks);
		return (0);
	}
